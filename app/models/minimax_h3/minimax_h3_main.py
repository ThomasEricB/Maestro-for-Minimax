...
# Read the AdaLN curve table's real dimensions from the checkpoint instead of
# assuming the constants below. Different publishers prune H3 with different
# curve settings — DeepBeepMeep's files are 1001x64, Comfy-Org's are 1025x8 —
# and building the network from the wrong pair fails to load. Costs one
# safetensors header read.
PROBE_CHECKPOINT_ARCHITECTURE = True
...
def probe_h3_checkpoint(filename):
    ...
    # A model may ship as a list of shards; the AdaLN curve table lives in
    # whichever one holds the top-level tensors, so scan until it is found.
    if isinstance(filename, (list, tuple)):
        for one in filename:
            result = probe_h3_checkpoint(one)
            if result["compressed_modulation"]:
                return result
...
def _fuse_weight_norm(module):
    """Collapse weight-norm parametrizations so the module expects a plain `.weight`.

    The audio VAE applies `weight_norm`, which splits each conv weight into a
    magnitude/direction pair. Publishers differ on which form they ship:
    DeepBeepMeep keeps the pair, Comfy-Org ships the already-fused weight.
    For a fused checkpoint the parametrization has to come off, or every
    weight-normed conv reports its two `original0`/`original1` tensors missing.
    """
    from torch.nn.utils import parametrize

    for submodule in module.modules():
        if parametrize.is_parametrized(submodule, "weight"):
            # leave_parametrized=True is required: weight_norm parametrizes in
            # terms of two tensors, and torch refuses to drop that without
            # collapsing it. On the meta device this only allocates shape
            # metadata — the values come from the checkpoint straight after.
            parametrize.remove_parametrizations(submodule, "weight", leave_parametrized=True)


def _audio_vae_is_fused(filename):
    """True when the checkpoint stores fused conv weights (no weight-norm pair)."""
    from safetensors import safe_open

    with safe_open(filename, framework="pt") as reader:
        for key in reader.keys():
            if key.endswith((".weight_g", ".weight_v")) or "parametrizations.weight.original" in key:
                return False
    return True
...
def _load_audio_vae(filename):
    ...
    fused = _audio_vae_is_fused(filename)
    if fused:
        _fuse_weight_norm(vae)
